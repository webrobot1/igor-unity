...
Момент ухода (`leaving_step`) несёт напоминание об ОСТАВЛЕННОМ браузере: ярлык считается по
самому событию, транскрипта не требует.
...
def leaving_step(event):
    ...
    (прогон тестов, анализатор, фоновая команда), `wait` — вопрос человеку. Шаг не уводит —
    пустая строка.

    `wait` стоит особняком по СРОКУ: прочие шаги кончаются сами, а окно вопроса ждёт человека и
    не кончится, пока тот не ответит — хоть часами. Потому удерживаемый разделяемый ресурс
    (браузер, запущенная игра) на нём не напоминают оставить, а освобождают, как на завершении
    ответа. Ярлык считается по самому событию: у окна вызывающий уходит от ЛЮБОГО инструмента,
    и исключения «та же работа продолжится» у него не бывает.
