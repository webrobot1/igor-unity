...
# Правится парно с knowledge-place «Правки конфигурации проекта».
...
# Зона владельца сводов — НОСИТЕЛИ, названные knowledge-place «Правки конфигурации проекта»: своды,
# `CLAUDE.md`, документация проекта, внешние MCP-инструкции бандла. Расширением имени она не
# задаётся: markdown лежит и вне её — артефакт прогона в каталоге инструмента, черновик, README
# чужого репозитория внутри дерева (git-baseline `storage/`, снапшот чужой игры, установленное
# окружение). Такой файл принадлежит тому, кто его завёл, и владения не открывает. Регистр
# расширения зоны не меняет: `docs/*.MD` — та же документация проекта.
...
def owners_of(path):
    ...
    Порядок ветвей значим: `.claude/agents/qa-server.md` принадлежит главной сессии, хотя и оканчивается `.md`."""
...
def refuse(pairs, agent, channel, point=None):
    ...
    deny("Отклонено правилом проекта (knowledge-place «Правки конфигурации проекта»): "
