	...
	public abstract class MapController : ConnectController 
	{
		...
		protected virtual void HandleData<P, E>(Recive<P, E> recive) where P : EntityRecive where E : EntityRecive
		{
			if (recive.sides != null)
			{
				...
				// Метки переходов зависят от того, какие карты сейчас стоят рядом: цель, достижимая границей,
				// метки не получает. Набор сторон сменился — пере-собираем метки уже выложенных карт, сами
				// карты при этом переиспользуются и повторно не грузятся.
				MapDecodeModel.RebuildWarpLayers();
