...
namespace Mmogick
{
    public class SigninController : BaseController
    {
		...
		/// контракта (<see cref="SigninRecive"/>) сервер обязан прислать всегда и, кроме перечня команд
		/// (пустой законен), непустыми: ими клиент повторяет серверный расчёт шага, резолвит позу покоя,
		/// рисует ячейки экипировки и отбирает команды, которые вправе слать, а своих умолчаний у него
		/// нет — подставленное значение молча увело бы клиент от сервера.
		///
		...
		private static string Contract(SigninRecive data)
		{
			...
			if (data.public_event == null)
				return "не пришёл перечень команд игрока (public_event)";
		...
		private IEnumerator LoadMain(SigninRecive data)
		{
			...
			else
			{
				...
				// Перечень команд — ДО загрузки игровой сцены, не вместе с полями пакета ниже: по нему элементы интерфейса
				// гасят себя в своём Awake (GameElementMarker), а Awake отрабатывает при загрузке сцены. Контракт поля
