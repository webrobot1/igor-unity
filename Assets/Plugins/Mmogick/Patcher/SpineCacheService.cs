...
namespace Mmogick
{
	...
	public static class SpineCacheService
	{
		...
		public class SlotAnchor
		{
			...
			/// <summary>
			/// Множитель размера надетого предмета в этом слоте: подгонка под конкретный слот и скелет.
			/// Саму величину задаёт КАРТИНКА предмета — кусок садится на скелет в натуральный размер своих
			/// пикселей (единица скелета есть пиксель графики), и множитель считается от него.
			/// Серверная доля клетки у набора картинок (<see cref="AnimationCacheService.PrefabEntry.size"/>)
			/// сюда не входит вовсе: она про размер САМОЙ сущности — предмета, лежащего на земле, — а не
			/// про кусок на чужом скелете.
			/// </summary>
