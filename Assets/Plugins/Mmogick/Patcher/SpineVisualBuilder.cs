...
{
	/// <summary>
	/// Сборка визуала сущности из скелета Spine — то же место в жизни сущности, что у сборки из дерева
	/// Spriter, но собирать почти нечего: рантайм сам строит скелет по данным, нам остаётся размер и клип.
	///
	...
	///
	/// Высота тела приводится к клетке: сервер объявляет её своим размером, и в единицах скелета она у
	/// каждого своя. Размера нет — берём габариты позы покоя, они у скелета есть всегда.
	/// </summary>
	public static class SpineVisualBuilder
	{
		...
		/// которые он сейчас показывает: спрайтов у скелета нет вовсе, он рисуется мешем, и общий замер по
		/// спрайтам (<see cref="AnimationCacheService.TryGetVisibleBounds"/>) на нём даёт пустоту.
		...
		/// <summary>
		/// Тело в клетку и по месту. Правила те же, что у сборки из дерева Spriter, — иначе одна и та же
		/// сущность на двух путях выглядела бы разного размера и стояла бы по-разному:
		///
		/// РАЗМЕР — по БОЛЬШЕЙ стороне габаритов, не по высоте: вытянутый поперёк (краб, корабль) иначе вылез
		/// бы за клетку шириной. Сервер объявляет свой размер — берём его, он точнее замера.
		///
		...
		/// фигура то вытягивается, то сжимается, и один момент дал бы случайный размер (сборка из дерева ради
		/// того же усредняла восемь идущих подряд кадров — скелету ждать их не нужно, поза считается сразу).
		/// </summary>
		private static void Fit(GameObject go, Transform child, SkeletonAnimation animation, float? serverSize)
		{
			...
			float span = serverSize.HasValue && serverSize.Value > 0.0001f
				? serverSize.Value
			...
			float scale = TARGET_HEIGHT / (parent * span);
