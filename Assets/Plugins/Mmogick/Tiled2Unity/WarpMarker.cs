	...
	public class WarpMarker : MonoBehaviour
	{
		...
		public static void BuildLayer(Transform grid, Map map, int sortingOrder)
		{
			...
			foreach (Layer layer in map.layer.Values)
			{
				...
				{
					// Переход задаёт СОБСТВЕННЫЙ класс объекта — так же, как его различает сервер: класс слоя
					// несёт лишь коллизии. Разметке, легшей на бесшовную границу, сервер ставит пустой класс,
					// и переходом она быть перестаёт.
					if (obj.type != warpClass)
						continue;
