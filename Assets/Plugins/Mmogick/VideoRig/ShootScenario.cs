...
namespace Mmogick.VideoRig
{
    ...
    ///            живое: через тот же источник ввода, который читают и игра, и EventSystem. Без цели
    ///            нажатие идёт по нынешнему месту указателя.
    ...
    ///
    /// Цель у pointer и click задаётся РОВНО одним полем: ui, panel (вместе с key), map либо entity.
    /// </summary>
    public class ShootAction
    {
        ...
        /// <summary>
        /// Цель: элемент панели интерфейса — слово панели; сам элемент называет <see cref="key"/>. Слово
        /// объявляет элемент (<see cref="IPanelElement"/>): у панели, показывающей компонент игрока, — slug
        /// компонента (actionbars, inventory, equip, spell_book — карточки и вкладки книги); у окон без
        /// такого компонента — слово оснастки: loot у ячеек окна добычи и лавки, у сеток окна сведений о
        /// цели — slug компонента цели с началом <see cref="InfoIcon.PANEL_PREFIX"/> (info_spell_book,
        /// info_loot_table). Имя объекта сцены словом и ключом не служит. Берётся центр области элемента.
        /// </summary>
        public string panel;

        /// <summary>
        /// Ключ элемента панели, всегда строкой: то, что элемент показывает и чем шлёт команду — номер
        /// слота либо ячейки, код слота экипировки, код заклинания, стихия вкладки, код префаба иконки.
        /// Без <see cref="panel"/> не бывает.
        /// </summary>
        public string key;
