...
namespace MyFantasy
{
	public class Websocket 
	{
		...
		/// <summary>
		/// структура  Recive которая должна обрабатвать наш ответ от севера
		/// </summary>
		private Type reciveType;

		/// <summary>
		/// Открытие TCP соединения
		/// </summary>
		/// <param name="command_pause">Пауза в секундах между командами для предотвращения даблкликов</param>
		public Websocket(Recive recive, string host, string player_key, string token, float command_pause = 0.15f)
		{
			errors.Clear();
			...
			try
			{
				...
				this.reciveType = recive.GetType();
