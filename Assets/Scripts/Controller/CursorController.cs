...
namespace Mmogick
{
    public abstract class CursorController : LootWindowController
    {
        ...
        /// <summary>
        /// Кольцо-подсветка кликабельной сущности под курсором — контейнера добычи, лежащей вещи, живого
        /// врага либо NPC. Один переиспользуемый world-объект на сцене (НЕ создаём per-frame): двигаем к
        /// наведённой сущности, прячем когда её нет.
        /// </summary>
        [SerializeField]
                ...
                else if (!overUi)
                {
                    ...
                    else
                    {
                        ...
                        if (new_target != null)
                        {
                            ...
                            // правилом, что hover-кольцо (ClickableAtScreen) — куда подсветили, туда и
                            // кликнули; кучей лежат только тела, у одиночного сундука выбор тривиален.
                            // Кликом берём ту же сущность, что подсвечена кольцом; контейнером она бывает не всегда —
                            // живой враг и лежащая вещь идут своими ветками ниже.
                            ObjectModel container = ClickableAtScreen(InputSource.MousePosition) as ObjectModel;
                            if (container != null && !LootWindowController.IsContainer(container))
                                container = null;
        ...
        ///
        /// Открыт наружу для оснастки съёмки роликов: найденную цель она сверяет с тем, что под точкой,
        /// этим же лучом и с этими же исключениями — своя копия правила разошлась бы с нажатием молча.
        /// </summary>
        public GameObject ScreenUiAtPointer(Vector3 screenPos)
        {
        ...
        // множитель кольца к рендер-границам сущности: 1.0 — ровно по спрайту (прозрачные поля текстуры
        // сами дают небольшой зазор). Держать МЕНЬШЕ зазора кликабельного коллайдера трупа
        // (ObjectModel.CORPSE_HIT_GAP) — кольцо лежит ВНУТРИ хит-области, клик по кольцу попадает по цели.
        private const float HIGHLIGHT_GAP = 1.0f;

        /// <summary>
        /// Hover-фидбек: навёл курсор на сущность, по которой пойдёт клик (контейнер добычи, лежащая вещь,
        /// живой враг либо NPC) — кольцо ВОКРУГ её спрайта: сигнал «кликабельно» и отличие цели от пустой
        /// земли под курсором. Курсор ушёл — скрыть. Один переиспользуемый объект (двигаем/масштабируем,
        /// не пересоздаём).
        /// </summary>
        private void UpdateHoverHighlight(Vector3 screenPos)
        {
            ...
            EntityModel hovered = ScreenUiAtPointer(screenPos) != null ? null : ClickableAtScreen(screenPos);
            if (hovered != null)
            {
        ...
        /// <summary>
        /// Центрирует и масштабирует кольцо ПОД конкретную сущность: центр и РАЗМЕРЫ по суммарным рендер-
        /// границам её спрайтов (EntityModel.TryGetVisualBounds) — облегает тело по ОБЕИМ осям (эллипс под
        /// аспект спрайта), а не описанной окружностью по большей стороне: у вытянутого тела окружность по
        /// большей стороне заметно шире его самого (жалоба «круг велик»). Те же границы использует ObjectModel
        /// для подгонки кликабельного коллайдера трупа — кольцо и хит-область совпадают. Нет спрайтов (редко)
        /// — по позиции.
        /// </summary>
        ...
        /// <summary>
        /// Сущность под курсором, по которой пойдёт клик, — контейнер добычи, лежащая вещь либо живой
        /// враг/NPC. Первая значимая (НЕ-игрок) сущность по лучу решает тип взаимодействия. Контейнер:
        /// среди ВСЕХ контейнеров под курсором выбираем БЛИЖАЙШИЙ к точке курсора (центры визуальных
        /// границ) — у сваленных в кучу тел коллайдеры перекрываются, а порядок RaycastAll при точечном
        /// луче недетерминирован, и без выбора по дистанции кольцо «прыгало бы» на соседнее тело. Прочее
        /// кликабельное берётся как есть: оно под курсором одно.
        /// Тем же методом цель выбирает и клик (Update) — подсветка и клик всегда об одной сущности.
        /// </summary>
        private EntityModel ClickableAtScreen(Vector3 screenPos)
        {
            ...
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                if (!containerZone)
                {
                    // Сверху живой либо предмет: клик уйдёт по НЕМУ, до контейнеров под ним дело не дойдёт —
                    // его и отдаём (кольцо обещает ровно тот клик, который случится). Клика своего у него нет
                    // (портал, алтарь, труп без добычи) — там клик равен клику по земле, цели нет.
                    if (!container)
                        return HasOwnClickAction(e) ? e : null;
                    containerZone = true;
                }
