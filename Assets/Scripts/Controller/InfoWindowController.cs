...
namespace Mmogick
{
    ...
    abstract public class InfoWindowController : DebugPanelController
    {
        ...
        ///
        /// Значок компонента заменяет собой ту часть строки, которая называет свойство: у строки с
        /// подписью — саму подпись (рядом остаётся значение), у строки-фразы — фразу целиком. Заменённое
        /// уходит в подсказку значка, и без неё значок ничего бы не сказал. Взять его негде лишь у
        /// команды: компонента у неё нет, и такая строка всегда остаётся текстовой. Берём SLUG компонента,
        /// а не готовый значок: перечень перебирается на каждом пакете, чтение картинки и сборка скелета
        /// же — работа для смены набора, и делает её сам пункт списка.
        /// </summary>
        private static List<InfoRow> Collect(InfoLine[] lines, ObjectModel subject, out bool allTypical)
        {
            ...
            foreach (InfoLine line in lines)
            {
                ...
                // Форм значка две, и компонент несёт ровно одну: картинку архива либо анимацию существа.
                // Строке довольно любой — ставит её сам пункт списка, здесь решается лишь, будет ли значок.
                string icon = iconKey != null
                    && (ComponentCacheService.GetImage(iconKey) != null || ComponentCacheService.GetAnimation(iconKey) != null)
                    ? iconKey : null;
        ...
        private static float? Shared(ObjectModel subject, string slug)
        {
            ...
            if (shared.Type != JTokenType.Integer && shared.Type != JTokenType.Float)
            {
                ...
                Error("Окно сведений: значение компонента " + slug + " у префаба " + subject.prefab
                    + " не число (" + shared.Type + ")");
                return null;
            }
