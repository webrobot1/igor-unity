...
namespace Mmogick
{
	...
	abstract public class LootWindowController : ActionBarsController
	{
		...
		// Имена компонентов добычи — общие для клиента, лежат у модели существа (EnemyModel).
		// Ценника (trade) в этом признаке НЕТ намеренно: компонент со структурным значением платформа заводит
		...
		private const string COMPONENT_LOOT = EnemyModel.COMPONENT_LOOT;
		private const string COMPONENT_LOOT_TABLE = EnemyModel.COMPONENT_LOOT_TABLE;
