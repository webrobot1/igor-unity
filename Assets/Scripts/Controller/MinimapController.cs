...
namespace Mmogick
{
    ...
    abstract public class MinimapController : PlayerController
    {
        ...
        /// <summary>
        /// UI-панель мини-карты целиком — подложка, картинки карт, точки, рамка. Её активность И ЕСТЬ
        /// состояние мини-карты — отдельного флага видимости нет: источник — настройка игрока (компонент
        /// settings, ключ minimap).
        ///
        /// Первым потомком панели лежит непрозрачная тёмная подложка (объект сцены UI/Minimap/Background;
        /// кодом она не трогается). Нарисованная картинка карты прозрачна везде, где карты нет, — за контуром
        /// острова, в проёмах между соседями, — и без подложки сквозь панель виден игровой мир под ней.
        /// Закрашивать прозрачное в самой картинке нельзя: её же показывает обзорная карта мира, а там
        /// под картами лежит своя подложка цвета окна (<see cref="WorldMapController"/>).
        /// </summary>
        [SerializeField]
        ...
        ///
        /// Число общее с обзорной картой мира, а клетка там мельче тем сильнее, чем крупнее охват мира:
        /// на карте в сотни клеток одни лишь клетки дают метки в считанные пиксели, и различимость их
        /// держит не это число, а нижний предел размера у самой обзорной карты
        /// (WorldMapController.MARKER_MIN_PIXELS). Крупная точка радара при этом не врёт: она показывает,
        /// что сущность стоит на соседней клетке, — метки соседей и должны соприкасаться.
        /// </summary>
        ...
        /// Карта вписана целиком (<see cref="DrawnWhole"/>): фон закреплён и растянут на всю панель по обеим
        /// осям РАЗДЕЛЬНО (аспект карты с аспектом панели не совпадает), потому клетка карты переводится в
        /// долю ширины и высоты порознь, тем же преобразованием, что и сама её картинка
        /// (<see cref="DrawSoleMinimapTile"/>). Клетку даёт MapImageCell: картинка считает клетки от своего
        /// угла, сцена — от середины клетки и от ног сущности.
        /// Окрестность вокруг игрока: фон панорамируется вокруг него, потому считается РАЗНОСТЬ позиций
        /// сцены — перевод в клетки картинки тут не нужен, он сдвигает обе точки одинаково и в разности
        /// пропадает.
        /// </summary>
        ...
        private void UpdateMinimapMaps(Vector3 playerPos, IReadOnlyDictionary<int, TileCacheService.CachedMap> maps, TileCacheService.CachedMap current)
        {
            ...
            // значит, что сервер ещё не прислал его: до этого фон не масштабируем. Радиус нужен обеим
            // веткам ниже — и масштабу, и выбору между ними, потому спрашивается до развилки.
            float halfPx = markerArea.rect.height * 0.5f;
            if (halfPx <= 0f || player.lifeRadius <= 0)
                return;

            // Карта без места в раскладке открытого мира стоит на радаре ОДНА: где лежат её соседи
            // относительно неё, не знает никто — раскладка их и держит. Снимаем их сразу, мозаика прежней
            // карты тут не продолжается; тот же отбор повторён фильтром основного цикла ниже.
            if (!current.hasOpenworldPosition)
                ReleaseStaleMinimapTiles(maps, player.map);

            // Карта, помещающаяся в охват радара целиком, вписывается в панель целиком — своей формулы у
            // неё нет (см. DrawSoleMinimapTile).
            if (DrawnWhole(current, player.lifeRadius))
            {
                DrawSoleMinimapTile();
                return;
            }
            ...
            foreach (KeyValuePair<int, TileCacheService.CachedMap> pair in maps)
            {
                ...
                // Отбор радара строже на одном входе: под картой без места в раскладке соседей нет вовсе
                // (см. выше) — радар панорамирует вокруг игрока и класть их ему некуда, а обзорная карта
                // раскладку мира показывает и без него.
                bool inLayout = current.hasOpenworldPosition
                    ? TileCacheService.InWorldLayout(pair.Value, pair.Key, player.map)
                    : pair.Key == player.map;
        ...
        /// от нуля и выше остаётся только эта карта (карта без места в раскладке стоит на радаре одна),
        /// иначе — все карты набора <paramref name="maps"/> (ушедшие из него остались в прежнем мире).
        ///
        ...
        /// <summary>
        /// Вписывается ли карта в панель целиком, а не показывается окрестностью вокруг игрока. Вписывается
        /// та, что помещается в охват радара сама — сторона панели соответствует двум радиусам жизни, — и
        /// места в раскладке открытого мира не имеет: соседей у такой карты нет, делить панель не с кем, и
        /// мелкий кусок в углу ей ни к чему.
        ///
        /// Крупная карта вне раскладки (остров, подземелье) идёт общим порядком, наравне с открытым миром:
        /// вписанная целиком, она показывала бы местность далеко за границей живого — за радиусом жизни
        /// сервер механик не считает, — врала бы пропорциями (панель квадратная, карта нет) и не слушалась
        /// бы настройки обзора вовсе, ведь размер карты от неё не зависит.
        ///
        /// Одно условие на оба показа радара — фон (<see cref="UpdateMinimapMaps"/>) и точки
        /// (<see cref="UpdateMarkers"/>): разойдись они, точки легли бы в масштабе, которого нет у фона.
        /// </summary>
        private static bool DrawnWhole(TileCacheService.CachedMap map, int lifeRadius)
        {
            return !map.hasOpenworldPosition
                && map.width  <= lifeRadius * 2
                && map.height <= lifeRadius * 2;
        }

        /// <summary>
        /// Рисует единственный тайл радара — карту, вписываемую целиком (<see cref="DrawnWhole"/>),
        /// растягивая её картинку на ВСЮ панель: комната заполняет зону видимости радара целиком, пропорции
        /// НЕ сохраняются (contain здесь не нужен, искажение допустимо и ожидаемо). Позиция и размер —
        /// фиксированные константы панели, от позиции игрока не зависят: этот тайл не участвует в общей
        /// per-frame формуле открытого мира (ранний return в UpdateMinimapMaps не даёт основному циклу его
        /// коснуться) — иначе он либо не двигался бы синхронно с формулой, либо съезжал бы и вылезал за
        /// рамку панели при ходьбе. Прочие тайлы, оставшиеся от прежнего состояния (мозаика открытого мира
        /// либо другая посещённая комната), к этому моменту уже сняты
        /// (<see cref="ReleaseStaleMinimapTiles"/>) — рядом с вписанной картой им не место.
        /// </summary>
        ...
        /// — окрестность вокруг игрока панорамируется → игрок неподвижен в центре панели,
        ///   все прочие точки — по разнице их мировой позиции и позиции игрока;
        /// — карта, вписанная целиком (<see cref="DrawSoleMinimapTile"/>), закреплена и растянута на всю
        ///   панель → наоборот, ПОЛОЖЕНИЕ игрока внутри неё решает, куда лечь его точке (в центре панели он
        ///   только тогда, когда физически стоит в середине карты), а точки прочих сущностей кладутся по
        ...
        private void UpdateMarkers(Vector3 playerPos, TileCacheService.CachedMap current)
        {
            ...
            _panelWholeMap = current != null && DrawnWhole(current, player.lifeRadius);
            ...
            if (_panelWholeMap)
            {
                // Отсекать нечего: на панели лежит вся карта, а сама она в охват радара помещается
                // (<see cref="DrawnWhole"/>) — дальше границы жизни точке тут просто неоткуда взяться.
                cullHalfSize = null;

                // Точка круглая, а вписанная карта растянута по осям РАЗДЕЛЬНО — размер выводим от одного
                // коэффициента, и взят коэффициент ВЫСОТЫ (пикселей панели на клетку карты по вертикали):
                // тем же измерением панели меряет себя ветка окрестности ниже (halfPx — половина высоты),
                // потому точка не меняет величины при переходе из комнаты в открытый мир.
                markerSize = MARKER_TILES * panelSize.y / current.height;
            }
            else
            {
                // Окрестность вокруг игрока: он неподвижен в центре, точки вне панели гасятся. Граница
                // панели и есть граница жизни — панель радиусом жизни и масштабирована; ею же отсекаются
                // переходы карты, которым до жизни дела нет. Масштаб тот же, что у фона в
                // UpdateMinimapMaps, иначе точки разъедутся с картинкой.
