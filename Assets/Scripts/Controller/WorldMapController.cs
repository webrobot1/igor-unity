...
namespace Mmogick
{
    ...
    abstract public class WorldMapController : InfoWindowController
    {
        ...
        /// <summary>
        /// Нижний предел размера метки в пикселях ЭКРАНА. Клетка тем мельче, чем крупнее охват мира, и на
        /// карте в сотни клеток метка по клеткам выходит в считанные пиксели: на пёстром рисунке её не
        /// найти, а метка игрока — единственное, что он ищет на карте глазами. Взята величина обычной метки
        /// радара (там на клетку приходится около семи пикселей панели): на обоих показах метка тогда одного
        /// порядка, и глаз, привыкший к радару, находит её и здесь.
        /// </summary>
        private const float MARKER_MIN_PIXELS = 12f;
        ...
        /// окно (<see cref="WorldMapScale"/>), но не мельче <see cref="MARKER_MIN_PIXELS"/> на экране.
        /// Своего числа в клетках обзорная карта не держит: величина точек задана одним местом на оба показа.
        /// Масштаб здесь на единичном увеличении — приближение растит метки вместе с картой
        /// (см. <see cref="ApplyWorldMapZoom"/>), потому размер и задан относительно раскладки.
        ///
        /// Предел делится на увеличение: раскладка масштабируется контейнером целиком, и в его собственных
        /// единицах предел рос бы вместе с картой — приближённая метка закрывала бы собой озеро. Делением
        /// предел остаётся экранным: он держит метку различимой на отдалении и отпускает её, едва размер по
        /// клеткам сам перерастёт эту границу.
        ///
        /// Предел стоит на ОБЩЕМ размере, не на итоговом размере каждой метки: своя метка крупнее прочих
        /// множителем (<see cref="MinimapController.PLAYER_MARKER_SCALE"/>), и упрись обе в один предел —
        /// на крупных картах они сравнялись бы, а своё место игрок находит именно по разнице величины.
        /// </summary>
        private float MarkerPixels(float scale)
        {
            return Mathf.Max(MARKER_TILES * scale, MARKER_MIN_PIXELS / worldMapZoom);
        }
        ...
        /// неё, и приближение не должно менять её размер ОТНОСИТЕЛЬНО места, которое она отмечает. Держит
        /// их у этого правила только нижний предел различимости (<see cref="MARKER_MIN_PIXELS"/>): он
        /// экранный, потому уже выложенные метки и пересчитываются тут — создаются они один раз на сборку
        /// и за увеличением сами не следят. Метка игрока считается каждый кадр
        /// (<see cref="PlaceWorldMapPlayer"/>) и в пересчёт не входит.
        /// </summary>
        private void ApplyWorldMapZoom()
        {
            ...
            float size = MarkerPixels(worldMapLayoutScale);
