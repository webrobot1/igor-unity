...
public class ObjectModel : MonoBehaviour
{
	...
	public void SetData(ObjectRecive data)
	{
        ...
        {
			// игрок уходит на другую карту
			// todo сверяться если карта - соседня локация то запустить корутину плавного перемещения на нее и проверки не пришли ли данные по соседней локации что существо там
			if(data.map_id != this.map_id)
            {
