...
namespace Mmogick
{
	...
	public class WorldBar : MonoBehaviour
	{
		...
		// Наибольший остаток, наблюдённый у ИДУЩЕГО срока: запасная длина шкалы, когда сервер длительности
		// не назвал. По ТЕКУЩЕМУ остатку её не считать — он убывает каждый кадр, доля выходит ровно единицей,
		// и полоска стоит полной весь срок. Значение принадлежит ОДНОМУ сроку и снимается вместе с показом
		// (<see cref="Hide"/>): сроков у одной сущности бывает несколько подряд (тело умерло, поднялось,
		// умерло снова), и длина прошлого занизила бы долю следующего.
		private double _peakRemain;

		/// <summary>
		/// Доля заполнения по сроку — готовый аргумент <c>part</c> для <see cref="Show"/>: remain — остаток
		/// срока в секундах, length — его длительность, названная сервером (null либо не больше нуля —
		/// группа её не рассылает).
		/// Названная длительность главнее наблюдения: она известна точно, а наблюдение начинается с того
		/// мига, когда сущность попала в поле зрения, — подошедший к чужому телу позже примет за полную
		/// шкалу свой первый увиденный остаток.
		/// </summary>
		public float Fraction(double remain, double? length)
		{
			if (remain > _peakRemain)
				_peakRemain = remain;

			double full = length.HasValue && length.Value > 0 ? length.Value : _peakRemain;
			return full > 0 ? (float)(remain / full) : 0f;
		}

		public void Hide()
		{
			// Показ окончен — наблюдение принадлежало прошедшему сроку, следующий копит своё.
			_peakRemain = 0;
