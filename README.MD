...
### Описание папок
	...
	Assets\Plugins						- плагины для работы с игры Unity с: фокусом WebGl, страницы отладки , websocket в тч webgl и др описанные в этой папке отдельно
	  ...
	  Assets\Plugins\SharpZipLib		    		- плагин распаковки игровых карт которые приходят с сервера
	  ...
	  Assets\Plugins\WebGLSupport		    		- набор плагинов нужных в сборке WebGl для: работы фокуса текстовых полей, изменение размера окна, Websocket соединения (в браузере), работы с отладкой в админ панели 
	  ...
	  Assets\Plugins\MyFantasy		   		- плагин для интеграции с онлайн-сервером 
	...
	Assets\WebGLTemplates  		- содержит шаблон WebGl для работы панели отладки  - отличается от стандартного наличием дополнительного JavaScript кода
	...
	Assets\Scripts\Struct  		- описание структур запросов и ответов от сервера
	  Recive\  			- стуктуры что нужно переопределить в плагине (в частности это стукруты игроков, монстров и объектов и их пользовательские поля)
	  Response\  			- структуры игровых механик (команд что отправляются с клиента) с данными что ожидает сервер (настраиваются в личном кабинете сервера)
...
4. Документировать реализацию (описание механики, взаимодействие клиент-сервер) в `/mnt/d/lessons/README.MD` с пометкой номера урока (lesson)
